...
public class WeatherSituationQueryFragmentTable extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        List<WeatherData> weatherDataList = WeatherSituationQueryFragment.weatherDataList;

        List<Integer> vs = Arrays.asList(40, 80, 80, 80, 80);
    ...
    private List<List<String>> createData(List<WeatherData> list) {
        ...
        datas.add(Arrays.asList("序号", "日期", "最高温度", "最底温度", "平均温度"));
        for (int i = 0; i < list.size(); i++) {
            WeatherData s = list.get(i);
            List<String> row = Arrays.asList(i + 1 + "", s.getDay(), s.getTeH() + "", s.getTeL() + "", s.getTeA() + "");
            datas.add(row);
        }
        return datas;
    }
