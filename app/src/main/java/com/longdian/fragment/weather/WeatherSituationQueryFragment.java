...
public class WeatherSituationQueryFragment extends BaseDatePickerFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        activity.setTopRightButton("bbb", chartRes, new TopBarBaseActivity.OnClickListener() {
            ...
            public void onClick() {
                if (isCurrentChart) {
                    replaceFragment(new WeatherSituationQueryFragmentTable());
                } else {
                    replaceFragment(new WeatherSituationQueryFragmentChart());
                }
    ...
    private void getData(String start, String end) {
        HoolaiHttpMethods.getInstance().weatherList(getActivity(), start, end, new ObserverOnNextAndErrorListener<List<WeatherData>>() {
            ...
            public void onNext(List<WeatherData> weatherDataList) {
                WeatherSituationQueryFragment.weatherDataList = weatherDataList;
                if (!isCurrentChart) {
                    replaceFragment(new WeatherSituationQueryFragmentTable());
                } else {
                    replaceFragment(new WeatherSituationQueryFragmentChart());
